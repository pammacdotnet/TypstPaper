...
#let form-well(gantt) = {
  ...
  gantt.insert(
    "style",
    merge-dictionary(default-style, gantt.at("style", default: (:))),
  )
...
#let draw-month-year-header(gantt, anchor-obj) = {
  ...
  create-header(
    ...
    date => align(center + horizon, strong[#months.at(date.month() - 1) #date.year()]),
    gantt.style.gridlines.months,
    "month-year",
  )(gantt, anchor-obj)
}
...
#let draw-3day-header(gantt, anchor-obj) = {
  import cetz.draw: *

  create-header(
    date => calc.rem-euclid(date.ordinal(), 3) == 0,
    (gantt, date, first) => {
      let (start, end, end-m1) = get-range(gantt)
      let end-date = date + duration(days: 3)
      if first {
        end-date -= duration(days: 3)
      }
      (calc.min(end-m1, end-date) - date).days()
    },
...
#let draw-header(gantt) = {
  ...
  for header in gantt.headers.rev() {
    if header == "month" {
      ...
      draw-month-year-header(gantt, anchor-obj)
      anchor-obj = "month-year-header"
    } else if header == "week" {
      ...
      draw-3day-header(gantt, anchor-obj)
      anchor-obj = "3day-header"
    } else if header == "day" {
