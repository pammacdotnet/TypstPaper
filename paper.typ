...
#let code-example(typ-file, caption, placement: none) = {
...
#figure(caption: [Ouput generated by the Typst code in @tab:LaTeXvTypst.])[
  #set text(
    font: "Cantarell",
    ligatures: false,
    size: 9pt,
...
#figure(
  image("typesetting systems.svg", width: 98%),
  kind: image,
  caption: "Evolution of Typessetting technologies",
  placement: top,
)
...
#code-example("unicode math example.typ", [Example of modern Unicode use in Typst and math expressions.])
...
For instance, the style rules necessary to produce @fig:affine are:
...
// Artículo de Typst as a Language
...
== Modules
Typst's other form of abstraction is modules. It conflates modules with files, which I think is reasonable for its aims. It's already dealing with all of typesetting, it should keep the language simple. There are three ways to `import` a module:

- `#import "mymodule.typ"` makes `mymodule` _visible_. Then you can write `mymodile.functionality` to access what it is
...
This is _almost_ well done, except for the fact that modules do not give the user/developer any way to mark items as
public or private. Thus every `let` statement in the whole module is exported (public). Some users#footnote("https://justinpombrio.net/2024/11/30/typst.html") have suggested some _solutions_:

- extending `set/show` to user-defined functions (e.g., set sequence(codon-sep: "-")) and improving `show` to tweak elements without reimplementing them entirely, like adding decorations to headings robustly.
...
The Neoplot#footnote("https://typst.app/universe/package/neoplot") package for Typst is a specialized tool designed to integrate Gnuplot @Janert16, a powerful open-source plotting engine, into Typst documents. It enables users to create high-quality graphs, charts, and mathematical visualizations directly within Typst by executing Gnuplot scripts or processing data files

#code-example("neoplot example.typ", "Neoplot WASM-based package example.")
...
Typst is a modern typesetting system that offers integrated support for bibliographic references, streamlining the citation process for users. It allows authors to include citations in their documents using the `cite` function or the `@key` syntax, referencing entries from bibliography files. Typst supports both BibLaTeX `.bib` files @Datta17 and Hayagriva `.yml` files as sources for bibliographic data. The system utilizes the Citation Style Language (CSL) to format citations and bibliographies @Fenner14, providing a wide range of built-in styles such as APA, MLA, IEEE, and Chicago. Users can also add custom CSL files to accommodate specific formatting requirements.

Hayagriva#footnote[https://github.com/typst/hayagriva] is a Rust-based bibliography management library developed to work seamlessly with Typst. It introduces a YAML-backed format for bibliographic entries and incorporates a CSL processor to format both in-text citations and reference lists. Hayagriva supports all styles provided in the official CSL repository, offering users access to over 2.600 citation styles. The integration of Hayagriva into Typst enhances the system's capability to handle complex citation formatting, ensuring that users can produce professionally styled documents with ease.

#figure(
  caption: [Example of a BibTeX entry and its Hayagriva equivalent for @Corbi23.],
  kind: image,
  grid(
    columns: (4fr, 4.5fr),
    rows: (auto, auto),
...
Typst offers robust support for mathematical expressions, providing a syntax that is both intuitive and powerful. To enter math mode, you simply enclose your expressions within dollar signs (`$...$`). For display-style equations, you can add spaces or newlines between the dollar signs and the content. Typst's math mode supports a wide range of symbols and functions, including Greek letters, operators, and more. Subscripts and superscripts are handled using the underscore (`_`) and caret (`^`) symbols, respectively. For example, `$x^2$` renders as $x^2$, and `$a_b$` renders as $a_b$. Additionally, Typst automatically scales delimiters like parentheses and brackets to fit their content, similar to LaTeX's `\left` and `\right` commands. This ensures that complex expressions are rendered clearly and accurately.

Beyond basic syntax, Typst allows for advanced customization of mathematical expressions. You can define matrices using the `mat` function, which accepts semicolon-separated rows and comma-separated columns, such as `$mat(1, 2; 3, 4)$` to render a 2x2 matrix. Typst also supports piecewise functions through the `cases` function, enabling you to define functions with multiple conditions in a clear format. Moreover, you can incorporate text within math expressions by enclosing it in double quotes, like `$x > 0 "if" y < 1$`. For users who prefer using Unicode symbols directly, Typst accommodates this as well, allowing for a more natural input of mathematical notation. Overall, Typst's math capabilities are designed to be both user-friendly and versatile, catering to a wide range of mathematical typesetting needs.
#code-example(
  "physica example.typ",
  [Example of advanced math with the Physica package#footnote[https://typst.app/universe/package/physica].],
)

```
lim_(h -> 0) (f(x + h) - f(x)) / h
```
XXX Ejemplos de undergrad math

== Drawings
https://typst.app/docs/reference/visualize/

#code-example("gradient stack.typ", "Gradient stack example. ")



Typst offers robust visualization capabilities through its extensible package ecosystem, enabling users to create high-quality plots and charts directly within their documents. Two prominent packages facilitating this are Lilaq#footnote("https://lilaq.org") and CeTZ-Plot#footnote("https://github.com/cetz-package/cetz-plot"). Lilaq provides a user-friendly interface for scientific data visualization, drawing inspiration from tools like Matplotlib and PGFplots. It emphasizes ease of use, allowing for quick creation of plots with minimal code, and supports features like customizable color cycles, axis configurations, and various plot types. The package integrates seamlessly with Typst's styling system, ensuring consistent formatting across documents.

#code-example("lilaq example.typ", "Example of a plot made with the Lilaq plotting package.")

On the other hand, CeTZ-Plot extends the CeTZ drawing library, offering functionalities for creating plots and charts within the CeTZ canvas environment. It supports various chart types, including pie charts, bar charts, and pyramids, and allows for detailed customization of plot elements. CeTZ-Plot is particularly suited for users who require intricate control over their visualizations and wish to integrate plots with other graphical elements.
...
#figure(
  ...
  placement: bottom,
  scope: "parent",
  caption: "A screenshot ot the Typst Universe website, highlighting the package CetZ.",
)
